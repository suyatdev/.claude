...
- **Fires:** PreToolUse on `Bash`, when `tool_input.command` **is** a `gh pr create` invocation. Classification runs through `python` `shlex` (which the hook already invokes), so the command is tokenized with correct shell-quoting semantics: after an optional `rtk` wrapper and any leading `NAME=VALUE` env-assignments (quoted or not), the actual command must be `gh pr create`. The phrase inside a commit message, echo, or quoted string is ignored, and a quoted-space env prefix cannot silently bypass the guard. Accepts the same chained-command gap as `git-guard`'s `^git` anchor (`foo && gh pr create` is not caught).
