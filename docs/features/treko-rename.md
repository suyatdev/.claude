...
- [x] 11. Verify criterion 13: load the prototype's two unmodified pages against the store this tool
      ...
      **Done 2026-08-22. All four checks pass**, each recorded with what it established and what
      it could not see — see §Verification, "Criterion 13". The important result is a **fifth
      observation that is not one of the four**: the prototype's board renders `Tasks 0` against
      our store, because it walks `f.stories[].tasks` while our features carry `tasks` directly.
...
- [x] 15. Close the card: set `phase: review`, and record here which of §Deferred card 2 inherits.

      **Done 2026-08-22.** `phase: review`; draft PR #64 carries tasks 0-14.

      **What card 2 inherits.** Card 2 is **the Ledger list and its persistent queue** — the only
      §Deferred bullet named for it. Everything below is measured on this branch, not predicted:

      - **The Ledger page does not exist in this repo.** `git ls-files` matches no `Ledger*`, and
        `STATIC_MANIFEST` (`server.py`) has no row for it. Criterion 13's Ledger evidence came from
        a **scratchpad fixture** holding the *prototype's* byte-identical `Ledger.dc.html`; nothing
        was committed. Card 2 ports the page — it does not adopt one.
      - **It is already proven to read our post-rename store correctly**, which is the real
        inheritance: against a `tracker-data.js` regenerated by this repo's `analyze.py` + `store.py`,
        the Ledger rendered `RUNS ON DISK 1`, `22 FEATURES`, `17 BRANCHES` and
        `Open 194 / 241 tasks merged` — every figure matching ground truth read out of the store.
        See §Verification, criterion 13 (c) and (d).
      - **The flat `f.tasks` shape must stay flat.** The Ledger reads it; the board does not, which
        is why the board renders `Tasks 0`. That is **card 3's** bug, and card 3's fix — nesting
        features under `stories[]` — would break card 2's page. The two cards touch one schema
        (ADR 0023) in opposite directions; whichever lands second must not silently win.
      - **`GET /` serves exactly one page**, `INDEX_FILE = "Treko.dc.html"` (`server.py:63`). A second
        page needs a routing decision, not just a manifest row and a `.dc.html` type entry.
      - **The seven `file://` console errors are not ours** — a control run on the prototype's own
        store reproduced all seven. Do not chase them when the ported Ledger shows them.
      - **The persistent queue's lifetime is an open structural question**, the same one §Deferred
        raises for card 4's panel persistence: the server's parent-pid watchdog exists so a
        full-permission channel cannot outlive its session, and a queue that persists past session
        close contradicts it. Card 2 decides this deliberately; it is not settled here.
      - **Treko still exits `2` outside cmux.** Auto-launch did not change that, so a Ledger reached
        by auto-launch is reachable only inside cmux.
...
### Criterion 13 — the prototype's unmodified pages against our store (2026-08-22)

**Fixture.** The prototype's `Task Tracker.dc.html` and `Ledger.dc.html` copied **byte-identical**
(SHA-256 verified against source; both mtime 2026-08-21 15:46:37, 64,218 and 16,847 bytes), with
their load-path dependencies, and exactly one thing changed: `tracker-data.js` regenerated by
**this repo's post-rename `analyze.py` + `store.py`**. Ground truth read out of that store, not
transcribed from this spec: `runs.length` **1**, `runs[0].features` **22**, `branches` **17**,
`waves` **1**.

**Two environments, because one tool could not reach the other.** The browser-automation tool
refuses `file://` URLs outright, so genuine `file://` loads were done with **headless Chrome**
(`--dump-dom`, `--enable-logging=stderr`) and the runtime introspection over
**`http://127.0.0.1`**. Which environment established which check is stated per check. The only
behavioural difference between them was found and characterised, not assumed — see (b).

**(a) `window.TRACKER_DATA` defined, `runs` a non-empty array — PASS.** Over `http://`:
`defined: true`, `Array.isArray(runs): true`, `runs.length: 1`. Envelope keys
`version, tool, generatedAt, repoUrls, runs`; `tool` is `task-tracker v0.4.1`, the exempt
contract value, intact. **The key is `generatedAt`** — confirming again that this spec's
`generated_at` is wrong. *Cannot see:* whether a `file://` load would differ; (b) shows the only
`file://` delta is an unrelated blocked fetch.

**(b) Zero uncaught console errors — PASS, with the `file://` delta settled by a control.**
Over `http://`, exactly one error appeared and it originates in a **browser extension**
(`chrome-extension://…installHook.js`, "Another version of Vue Devtools seems to be installed"),
not the page. Over genuine `file://`, **seven** errors appear: six SVG attribute parse failures
(`<path> attribute d: … "{{ e.d }}"`, `<circle> cx/cy: "{{ e.tx }}" / "{{ e.ty }}"`, at lines
220-221 and 243-244) and one `Access to fetch at 'file://…' has been blocked by CORS policy`.

**A control run settles the attribution instead of leaving it to inference.** The same page was
loaded from an identical fixture whose only difference is the **prototype's own**
`tracker-data.js` — and it produced **the same seven errors**. None is caused by our store: the
six are raw `{{ }}` placeholders the browser parses before the runtime substitutes them, and the
CORS one is `support.js:159`'s `fetch(location.href)`, which fires only when `window.__resources`
is absent — always true for the prototype, which ships no `vendor-resources.js`. *Cannot see:* an
error thrown after the capture window, or one swallowed by a `catch`.

**(c) Board run count and Ledger `runCount` both equal `runs.length` — PASS.** `runs.length` is
**1**; the board's sidebar lists exactly one run (`treko-ui-update … 2026-08-22T01:28:37Z`); the
Ledger renders **`RUNS ON DISK 1`**. Three independent readings, one number.

**(d) `features`, `branches`, `waves` each render ≥1 row — PASS, and each matches ground truth
exactly**, which is stronger than the criterion asks:

| section | store | board renders | Ledger renders |
|---|---|---|---|
| `features` | 22 | `What's left 22` | `22 FEATURES` |
| `branches` | 17 | `Branches 17` | `17 BRANCHES` |
| `waves` | 1 | `Merge order 1` | — |

**The finding that is not one of the four checks.** The board renders **`Tasks 0`**. Not a (d)
failure — (d) names only `features`, `branches`, `waves` — but it is the exact failure shape this
criterion exists to expose, so it is recorded rather than passed over. Cause, measured on both
stores rather than reasoned about:

| | our store | prototype's own store |
|---|---|---|
| `features[0]` keys | `name, meta, tasks` | `name, meta, rally, stories` |
| any feature has `stories` | **false** | **true** |
| board renders | `Tasks 0` | `Tasks 10` |

The board's task list walks `f.stories[].tasks`; our analyzer emits `f.tasks` directly, with no
story level. **The data is not missing** — all 22 features carry tasks, and the Ledger, which
reads the flat shape, correctly reports `Open 194 / 241 tasks merged`. The two prototype pages
disagree with each other about the schema. This is the gap §"The nested rows are not portable
yet" predicted; card 3 owns it, and its first task is the schema conversation, not the markup.

**What this criterion cannot see at all.** It exercises one store, from one repo, at one moment.
