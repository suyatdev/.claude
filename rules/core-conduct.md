...
**Never render a metric the payload cannot source.** When a requested display, report, or summary has no backing field in the data, the failure mode is not an error — it is a confident-looking number: it reads as a measurement while being a substitute that silently rots as the underlying facts change. Show the real measurement or show nothing; prefer the honest weaker metric and say plainly which one the data actually supports. The same discipline covers the schema itself — verify a field's real name and type before building on it, because a guessed one fails closed, and failing closed is indistinguishable from the feature being switched off.
