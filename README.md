...
- [x] Non-text contrast is guarded in both themes, on a named 23-token allowlist: the dots, border sides, shadows and SVG shapes that no check had ever looked at — 334 marks in dark and 347 in light — now fail the suite if a palette edit dulls them. Six tokens are held above 3:1, three known-too-faint ones are pinned at exactly their current ratio so a silent *repair* is as visible as a silent regression, and fourteen are exempt with a written reason each. Implementing WCAG 1.4.11 literally was measured and rejected: it fails 78% of the page, including 1px hairlines designed to be nearly invisible. A green run means those 23 tokens have not regressed — it does not mean the board is accessible, and every test docstring says so (ADR 0037; #80)
- [x] Treko's board is themeable: 27 hardcoded colours became a named token layer — 8 tokens at the tokenize commit, 52 distinct custom properties in the shipped file once the accent and neutral ramps landed — which is what makes the rest possible — a light theme, a Configuration drawer behind the gear (Appearance and Layout), and a sidebar you can drag from 190 to 440px and that remembers where you left it. The tokenize pass ships as a provable no-op in its own commit and the repaint in a second, so the colour change has a rollback point; before this branch nothing tested the page's appearance at all (ADR 0036; #79)
- [x] Worktree-location guard: the "work in a worktree, never the primary checkout" rule enforced in two layers that fail in opposite directions — `worktree-guard.sh` reads command text and refuses before git runs (fails open), and the `reference-transaction` git hook watches the ref update itself and fails closed on any `HEAD` write in a primary checkout. `create-worktree.sh` redirects new worktrees to `~/.worktrees/<repo>/<name>` rather than denying them. Ships in `log` mode — it records what it would have blocked and blocks nothing; arming it is a separate deliberate commit, as is layer 2's `core.hooksPath` install. Deliberately covers precision, not coverage: the Bash write surface is undecidable and layer 2 backstops `HEAD` moves only, so six working-tree commands remain layer 1's sole responsibility (ADR 0038; #83)
- [x] Secret-command guard: a `PreToolUse`/`Bash` hook that refuses a command naming a known secret-bearing dotfile (`~/.terminal_aliases`, `~/.zshrc`, `.env`, `credentials.json`, …) or dumping a full environment, written after a single session printed real secrets into its own transcript twice. There is **no permitted read shape** — the first version allowed a `grep -o` call on the theory that `-o` echoes only the match, and the observability judge falsified it: the caller picks the pattern, so `grep -o 'export .*'` reproduced the original leak through the one door left open, with 23 green tests none the wiser. The carve-out was removed whole rather than narrowed, `.env.example`/`.template`/`.sample` are exempt so ordinary work is not caught, and `SECRET_EXEMPT=<reason>` is the logged hatch the first version argued it did not need. Fails **open** — the inverse of its sibling `scan-secrets.sh`, armed in the same change after its "passes its tests" claim turned out to describe a suite that did not exist. Seven bypass shapes are measured, disclosed and pinned by `ALLOW` assertions — including that "any mention" really means "the path is a whole trailing component of a lexed argument" (`bash -c "cat ~/.zshrc"` blocks; adding `| head` allows, and so does `cat foo.zshrc`), and that a read inside a script file — the first leak's own form — is not covered at all (ADR 0039; #85)
- [x] Secret-gate approval: `SECRET_EXEMPT=` no longer clears a secret-command block on its own. The guard prints an approval id, and the flag is honoured only alongside a recorded grant keyed to the session **and** to a fingerprint of that one command, deleted on first use — one approval, one run, one command. The observability judge then falsified the first working version: the fingerprint deliberately excluded the `SECRET_EXEMPT` *value* ("you approve a command, not a wording"), but bash executes that value, so a backtick command substitution in the reason rode in on a grant issued for the plain read — reproduced end to end, same id, grant consumed. Hashing the value in cannot work (the id is issued before a reason exists), so the value is constrained instead, by an **allowlist** rather than a denylist of dangerous characters: a reason must now be one plain word. That narrows real behaviour — two previously passing assertions using quoted multi-word reasons were inverted rather than the rule widened to fit them. Still **not a security boundary**: the approval record is written from inside the session by the agent the gate constrains, so it is forgeable, and the load-bearing control remains the human typing `secret-gate override`. The guard's Known-gaps table stands at **nine** measured `ALLOW` shapes, each pinned by an assertion; two were found during this work and disclosed rather than fixed — an input redirection (`cat < ~/.zshrc` lexes to `['cat']`) and an unquoted `#`, which truncates the lexer six hooks share and is queued as its own card (ADR 0039; #85 for the guard itself)

- [x] Handoff-trim safety: the three handoff hooks used to order the model to shorten `.claude/session-state.md` with no copy kept — one session lost ~1,200 lines silently. Now every trim is preceded by a per-session snapshot, cut text is filed into a gitignored local archive that `memsearch` indexes (never deleted, never leaves the machine), a hook that cannot take the snapshot degrades its directive to append-only and says so, `[KEEP]` headings mark sections a `Stop` hook checks survived (block, re-inject, two strikes, then fail open loudly), session start reports whether that guard is alive, and an over-cap file is truncated and said rather than blanked. The observability judge falsified the first fix twice — a "snapshot was kept" message whose snapshot the next prompt overwrote, and split test runners that stayed green with a part file deleted — and both falsifiers now live in the suites (ADR 0045, ADR 0046; #105)
