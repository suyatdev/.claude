...
MSG_NO_PYTHON='worktree-guard: blocked — no python3 or python on PATH, so this tool payload could not be read at all. The guard cannot permit a request it cannot identify. This fires before the exemption list is reached — settings.json is not exempt here; and there is no in-session route around it either, because this refusal fires before the guard dispatches on tool name, so a Bash edit is refused exactly the same way. Restore python3 on PATH.'
