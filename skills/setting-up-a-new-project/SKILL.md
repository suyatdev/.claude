...
*Default:* human gate. *Why:* auto-merge sits in direct tension with the default-branch-safety rule in `rules/gates.md` and the tests-pass-first rule in `preparing-pull-requests`. It can be the right call for a low-stakes repo with strong tests, but it has to be a deliberate opt-in that a human chose, not a habit the repo slid into.
...
3. Create the repo's own `docs/features/` and `docs/decisions/` from the first commit — the feature card carries frontmatter, spec, and checklist; ADRs carry the structural decisions (see `managing-session-memory`). Record this setup register's answers under `docs/` so future sessions know it has run and where the answers live. **Do not create `CODING_MEMORY.md` or a `coding-memory/` directory** — that pair was a parallel record tree that duplicated `docs/` and has been retired (ADR 0031); a new repo should never start one.
