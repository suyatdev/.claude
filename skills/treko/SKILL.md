...
description: Use when surveying which feature cards and branches are in flight in a repo — deriving a run with the analyzer, reading the merge order it proposes, or launching the browser tracker UI and its control server. Not for editing a card's phase or writing a handoff (see managing-session-memory).
---

# Tracking Feature State

`task-tracker/` answers one question: **what is in flight in this repo, and in what order can it
...
```sh
python3 task-tracker/analyze.py <repo-root> --pretty
```
...
From the repo root, as a **direct child of the Claude session** — its own foreground shell, or the
harness's background-run mode, which keeps the process in the session's tree and its stderr captured:

```sh
python3 task-tracker/server.py --repo "$PWD"
```
...
The port is `8422`, owned by `PORTS.md` (`TASK_TRACKER_PORT` overrides). Open
...
(`TASK_TRACKER_IDLE_SECS`, floor 60s, cannot be disabled) or within ~5s of the session ending. There
...
| `port … is already in use` | Another session's server still holds it. Stop that one; this server will not probe for a free port. |
...
| `Task Tracker.dc.html has no <head>` | The token has nowhere to be injected, so the page would load unauthenticated. |
...
- "open the task tracker UI for this repo"
