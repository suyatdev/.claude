...
- **Event-based saves:** update it immediately after completing a major task, resolving a significant bug, or making a decision in any of three classes that must never go unrecorded — (a) a structural/architectural decision, (b) a change that affects business logic, or (c) a technological implementation that pivots the direction of a feature. These three are mandatory triggers: "it was a small change" does not exempt a decision that moved the product's behavior or direction.
- **Durable decision records (ADRs):** a decision in class (a), (b), or (c) also earns its own Architecture Decision Record under `docs/decisions/` — a short, numbered, immutable file stating the decision, the options weighed, why this one won, and the consequences. The mutable `CODING_MEMORY.md` index gets only a one-line pointer to it; a decision recorded solely in the index is one trimming away from losing its rationale. New repos scaffold `docs/decisions/` and its template via `setting-up-a-new-project`. When the decision has structure or a tradeoff space, embed a rendered Mermaid diagram in the ADR — see `diagramming-technical-docs`.
