...
- `dispatching-pane-agents` — routing a subagent across the three lanes (Explore/Plan in-process, judges always paned, workers under this session's `inline`/`panes max=N` policy); result-file contract, wait timeouts, fallback rules.
