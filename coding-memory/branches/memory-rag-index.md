...
- Final pre-merge fix wave (whole-branch review, model-switch/DB-lifecycle recovery seam): search() now checks db.model_mismatch after connect() and raises SystemExit instead of letting a raw sqlite3.OperationalError escape on a same-or-different-dim embed model swap; rename_repo() now guards on cfg.db_path.exists() before connect() so it can no longer silently create an empty DB and report zeros when no index exists; added run_index(full=True) rebuild coverage (dedupes, reprocesses instead of hash-diff-skipping) — RED-proven for the first two via a raw sqlite3.OperationalError and a silently-created DB respectively, green for all three after the fix
- PR #14 OPENED (2026-07-18): https://github.com/suyatdev/.claude/pull/14 — judge risk=low conf=high; verdict + 2nd parallel verdicts.jsonl reconcile committed post-PR
