...
| 8422 | .claude | Treko control server (`treko/server.py`) | Local dev process, session-scoped | Allocated 2026-08-09. Binds `127.0.0.1:8422` explicitly — never `0.0.0.0`; see §Security of `docs/features/tracking-feature-state.spec.md`. Chosen outside the 8000-8100 block mtg-wizard and snatch-bracket own. Exits with the session and on idle timeout — no daemon, no launchd, so a stale listener here means a leaked process. Override with `TREKO_PORT`; register the replacement here if the override becomes permanent. |
